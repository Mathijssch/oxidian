...
lazy_static! {
    static ref OBSIDIAN_LINK_RE: Regex =
        Regex::new(r"(?P<is_attachment>!?)\[{2}(?P<link>[^\[]*?)\]{2}").unwrap();
    static ref MD_LINK_RE: Regex =
        Regex::new(r"(?P<is_attachment>!?)\[(?P<alias>[^\]]*?)\]\((?P<target>[^\)]*?)\)").unwrap();
    static ref STANDARD_URL_RE: Regex = Regex::new(r"(https:\/\/|http:\/\/|www\.)(\S*)").unwrap();
}
...
pub fn find_obsidian_links(content: &str) -> Vec<Link> {
    OBSIDIAN_LINK_RE
        .captures_iter(content)
        .map(|capture| {
            Link::from_obsidian_link(&capture["link"], !capture["is_attachment"].is_empty())
                .unwrap()
        })
        .collect()
}

pub fn find_raw_links(content: &str) -> Vec<Link> {
    let links = STANDARD_URL_RE
        .captures_iter(content)
        .map(|capture| Link::from_raw(&capture[2], &capture[0]))
        .collect();
...
mod tests {
    ...
    fn test_multiple_links(content: &str, links: Vec<&str>) {
        ...
        for (found_link, given_link) in found_links.iter().zip(
            links
                .iter()
                .map(|link_str| Link::from_obsidian_link(link_str, false).unwrap()),
        ) {
    ...
    fn test_basic_link() {
        test_multiple_links(
            "some line with a simple [[link]] to another file.",
            vec!["link"],
        );
    }
    ...
    fn test_basic_link_subpath() {
        test_multiple_links(
            "some line with a simple [[ link#header ]] to another file.",
            vec![" link#header "],
        );
    }
    ...
    fn test_basic_link_alias() {
        test_multiple_links(
            "some line with a simple [[ link#header | alias ]] to another file.",
            vec![" link#header | alias "],
        );
    }
    ...
    fn test_basic_link_attachment() {
        ...
        assert_eq!(
            found_links[0].link_type(),
            LinkType::Attachment(FileType::Image)
        );
    }
    // More tests...
}
