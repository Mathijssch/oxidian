...
pub fn find_tags(content: &str) -> Vec<Tag> {
    let mut tags = OBSIDIAN_TAG_RE
        .captures_iter(content)
        .filter(|capture| {
            capture.get(0).map_or(false, |c| {
                content[c.start()..c.end()]
                    .chars()
...
mod tests {
    ...
    fn test_multiple_tags(content: &str, tags: Vec<Tag>) {
        ...
        found_tags
            .iter()
            .zip(tags.iter())
            .for_each(|(found_tag, given_tag)| {
                assert_eq!(found_tag, given_tag);
            });
    }
    ...
    fn test_basic_tag_with_space() {
        test_multiple_tags(
            "Something with a false # postiive and a #true_positive",
            vec![Tag {
                tag_path: "true_positive".to_string(),
                source: "#true_positive".to_string(),
            }],
        );
    }
    ...
    fn test_tag_special_characters() {
        ...
        test_multiple_tags(
            "A line with a #tag2 and #tag_2 and #tag3#tag4 and #tag\\5",
            vec![
                Tag {
                    tag_path: "tag_2".to_string(),
                    source: "#tag_2".to_string(),
                },
                Tag {
                    tag_path: "tag2".to_string(),
                    source: "#tag2".to_string(),
                },
                Tag {
                    tag_path: "tag3".to_string(),
                    source: "#tag3".to_string(),
                },
                Tag {
                    tag_path: "tag".to_string(),
                    source: "#tag".to_string(),
    ...
    fn valid_subtags() {
        // Note the order! The tags are sorted by length!
        test_multiple_tags(
            "A valid #root/sub1/sub2 tag",
            vec![Tag {
                tag_path: "root/sub1/sub2".to_string(),
                source: "#root/sub1/sub2".to_string(),
            }],
        );
    }
}
